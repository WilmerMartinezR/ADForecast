...
for(mW in 0:(ncol(datos)-6-W.Wide-h)){
  #mW = 0; mW = ncol(datos)-5-W.Wide-h
  fecha.ini <- names(datos)[6+mW]
  fecha.fin <- names(datos)[6+W.Wide+mW]
  
  output <- T_ModResiForestARp_direct(datos, fecha.ini, fecha.fin, h)
...
file.out <- paste("Fores",Country,"_ARp_direct_", outputForestE$fechaf[1], "_",                  
